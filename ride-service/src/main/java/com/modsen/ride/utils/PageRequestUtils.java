...
import com.modsen.ride.exception.IncorrectPageNumberException;
import com.modsen.ride.exception.IncorrectPageSizeException;
import com.modsen.ride.exception.IncorrectSortFieldNameException;
...
public class PageRequestUtils {
    private static final Predicate<String> IS_SORT_FIELD_EXIST_PREDICATE = sortField ->
            Arrays.stream(Ride.class.getDeclaredFields())
                    .map(Field::getName)
    ...
    public static void validatePageResponse(PageRequest pageRequest, Page<Ride> ridePage) {
        int actualPageQuantity = ridePage.getTotalPages();
        int requestedPageQuantity = pageRequest.getPageNumber() + 1;
        int pageSize = pageRequest.getPageSize();
        if (!ridePage.hasContent() && requestedPageQuantity > actualPageQuantity) {
